...
int	run_cmd(t_infos *info, t_cmd *str)
{
	...
	if (current->next == NULL) // the case of no pipe
	{
		...
		if (check_if_builtin(current) == 1) 	// if builtin
		{
			...
			// exec_builtin(current, info, str); // changed jaka 30 may
			g_status = exec_builtin(current, info, str); // changed jaka 30 may
			printf(GRN"    no pipe: g_status %d\n"RES, g_status); // added jaka, 30 may
		...
		while (current)
		{
			...
			if (pid == 0)
			{
				...
				if (check_if_builtin(current) != 0) 	// if builtin
				{
					...
					printf(GRN"    with pipe: g_status a %d\n"RES, g_status); // added jaka, 30 may
				}
				else // if library
				{
				...
				exit (g_status);  // in case of builtin, it should be quit // change jaka 30 may, to fix exit.c
			}
			else
			{
				if (current->fd_out > 1)
	...
	reset_fd(info);
	printf(GRN"    End of run_cmd: g_status %d\n"RES, g_status); // added jaka 30 may, trying to fix exit.c
	return(g_status);
}
